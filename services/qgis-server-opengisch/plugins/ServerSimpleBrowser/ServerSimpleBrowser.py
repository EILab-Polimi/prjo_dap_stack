...
class ServerSimpleFilter(QgsServerFilter):
    ...
    def get_url(self, request, params):
        ...
        QgsMessageLog.logMessage("ServerSimpleFilter - SCRIPT NAME")
        QgsMessageLog.logMessage(script_name)

        """ Force the script name to nginx route in order to work with qgisch Qgis server """
        script_name = "/ows/"

        url += script_name
        url += '?MAP=' + params.get('MAP', '')
        QgsMessageLog.logMessage("ServerSimpleFilter - URL")
